...
par(mfcol=c(2,2),type="n", mar=c(4,4,2,1), oma=c(0,0,2,0)) ### Creates the structure of the four graphs, filling in the columns first
...
legend("topright", legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),
       lwd=2, col = c("black","red","blue"),bty="n") ### box line set to "n" so that it doesn't show up  

### New Plot of Voltage vs. Days

with(dat2, plot(Voltage~Days, ### Create the desired graph
                type="l", ### This creates the line graph that is required
                xlab="datetime", ylab="Voltage"))

### New Plot of Global_reactive_power vs. Days

with(dat2, plot(Global_reactive_power~Days, ### Create the desired graph
                type="l", ### This creates the line graph that is required
